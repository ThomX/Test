...
éditer config.inc et écrire "COMPILE_CORE=1
COMPILE_SIMULATOR=0
COMPILE_SPS_IMAGE=1
COMPILE_ESPIA=0
COMPILE_FRELON=0
...
=> Non, ce n'est pas obligatoire mais très utile donc c'est par défaut. Voire les  attributs/propriétéss/commandes commençant par «config» (http://lima.blissgarden.org/applications/tango/doc/index.html#main-device-limaccds)

make config
Quand on exécute «make config» dans le répertoire Lima : «usr/bin/sip: not found» -> installer le paquet SIP4 (ne semble pas être indiqué dans la doc)
=> sera ajouté dans la doc
make install
